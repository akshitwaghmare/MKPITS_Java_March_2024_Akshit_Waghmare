...
select title, case when length(title) % 2 = 0 then 'even' else 'odd' end as EVEN_OR_ODD from film
#36. **Find all films with a rental rate greater than 3 and categorize them as 'High' or 'Low'.**
select *, case when rental_rate>3 then 'high' else 'low' end as HIGH_OR_LOW from film
...
select unicode(first_name) from actor
#40. **Convert the rental rate of each film to a binary string.**
select bin(rental_rate) from film char
